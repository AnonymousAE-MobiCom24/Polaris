...
delta_thrd_tol =[35, 30, 30, 34, 37, 33, 33, 35, 41] 
...
car_flag = True
car.set_akm_steering_angle(0, ctrl_car=False)
ID = 0
...
def detectMag():
    ...
    global ID, start_time_2
    no_tag_detection = True
    time_flag = False
    start_flag = False
    stop_flag = False
    steer_angle = 0
    ...
    try:
        while True:
            ...
            for i in range(num):
                ...
                # every ten points, update the start_raw_amp_x, start_raw_amp_y, start_raw_amp_z:
                if cnt % 10 == 0 and no_tag_detection:
                    temp_data = raw_result[cnt-5:cnt]
                    temp_x = [temp_data[ii][i+1][0] for ii in range(5)]
                    temp_y = [temp_data[ii][i+1][1] for ii in range(5)]
                    temp_z = [temp_data[ii][i+1][2] for ii in range(5)]
                    start_raw_amp_x[i] = np.mean(temp_x)
                    start_raw_amp_y[i] = np.mean(temp_y)
                    start_raw_amp_z[i] = np.mean(temp_z)
            if first == 0:
                for i in range(num):
                ...
                if sd_sig_tol[i][-1] < 0 and sd_sig_tol[i][-2] >= 0:
                    ...
                    if(car_flag):
                        for i_ind in range(num):
                            # if total slope_list_tol is larger than 30, start moving
            ...
            if(len(tag_info) == 3 and first_tag):
                no_tag_detection = True
                # speed: mm
                v = 100
                f = 50
                ...
                # ID += 1
                if within_hamming_distance(decode_info, "101101101"):
                    steer_angle = 45
                    car.set_car_motion(0.06, 0, 0)
                    car.set_akm_steering_angle(steer_angle, ctrl_car=False)
                    
                  
                    print("Start")
                    print("The heanding angle is: ", orientation)
                    start_flag = True
                ...
                # every 0.1s, update the steering angle
                ...
                # if within_hamming_distance(decode_info, "000000000"):
                #     time_flag = True
                #     print("Stop")
                #     car.set_car_motion(0.0, 0, 0)
                    # car.set_akm_steering_angle(0, ctrl_car=False)
                

                # if within_hamming_distance(decode_info, "011011011011011"):
                # if within_hamming_distance(decode_info, "101101101101101"):
        ...
        # reconstructing the tag layout
            if(len(tag_info) == 5 and not first_tag):
                no_tag_detection = True
                # speed: mm
                v = 100
                f = 50
                ...
                # ID += 1
                if within_hamming_distance(decode_info, "000000000000000", 5):
                    car.set_car_motion(0.0, 0, 0)
                    car.set_akm_steering_angle(0, ctrl_car=False)
                    start_time_2 = time.time()
                    time_flag = True
                ...
                # if within_hamming_distance(decode_info, "110110110110110"):
                #     car.set_car_motion(0.0, 0, 0)
                #     car.set_akm_steering_angle(0, ctrl_car=False)
                #     start_time_2 = time.time()
                #     time_flag = True
            ...
            # trajectory 1:
            if(start_flag and time.time() - start_time_2 > 0.1):
                steer_rad = np.radians(steer_angle)
                # velocity = car.get_motion_data()[0] * 100
                velocity = 6
                print("velocity: ", velocity)
                turn_radius = 45
                steer_rad += velocity / turn_radius * 0.1
                start_time_2 = time.time()
                steer_angle = np.degrees(steer_rad)
                print("steer_angle: ", steer_angle)
            if(start_flag and 62 < steer_angle < 63):
                steer_angle = -3
                car.set_car_motion(0.06, 0, 0)
                car.set_akm_steering_angle(steer_angle, ctrl_car=False)
            if(start_flag and  -2 < steer_angle < -1):
                steer_angle = -3
                car.set_car_motion(0.06, 0, 0)
                car.set_akm_steering_angle(steer_angle, ctrl_car=False)
                start_flag = False
            ...
            # if time_flag and time.time() - start_time_2 > 8.8:
            #     print("Right over")
            #     time_flag = False
            #     car.set_car_motion(0.06, 0.0, 0)
            #     car.set_akm_steering_angle(0, ctrl_car=False)
        ...
        # Raw data
        test.to_csv("RawData3_" + scenario + "_" + str(times) + ".csv")

        test_z = pd.DataFrame(columns=["THR_Z"], data=slope_thrd_tol)
        # Thresholds 
        test_z.to_csv("THR_tol3_" + scenario + "_" + str(times) + ".csv")
        ...
        for i in range(num):
            ...
            test_x.to_csv(
                'Smoothed_x_3' + scenario + "_" + str(times) + "_Sensor" + str(
                    i + 1) + '.csv')
            test_y = pd.DataFrame(columns=["Y data"], data=sy[i])
            test_y.to_csv(
                'Smoothed_y_3' + scenario + "_" + str(times) + "_Sensor" + str(
                    i + 1) + '.csv')
            test_z = pd.DataFrame(columns=["Z data"], data=sz[i])
            test_z.to_csv(
                'Smoothed_z_3' + scenario + "_" + str(times) + "_Sensor" + str(
                    i + 1) + '.csv')
            test_sig = pd.DataFrame(columns=["Signal data"], data=s_sig_tol[i])
            test_sig.to_csv(
                'Smoothed_sig_3' + scenario + "_" + str(times) + "_Sensor" + str(
                    i + 1) + '.csv')
...
def within_hamming_distance(code1, code2, max_distance=2):
    """
...
def gray_code(num):
    """
    Function to generate Gray code for a given number.
    """
    return num ^ (num >> 1)
...
def decode_phase(phase, N):
    ...
    # Convert the index to Gray code
    gray_index = gray_code(index % N)
    
    # Calculate the number of bits per phase based on N
    num_bits = int(np.ceil(np.log2(N)))
    
    # Convert the Gray index to binary code
    return format(gray_index, f'0{num_bits}b')
